...
class RecorrenciasService {
  RecorrenciasService({
    ...
    RecorrenciasConfigService? configService,
  }) : _repository = repository,
       _recorrenciaDespesaService = recorrenciaDespesaService,
       _configService = configService ?? RecorrenciasConfigService();
  ...
  Stream<List<RecorrenciaAtiva>> streamRecorrenciasAtivas() {
    return Rx.combineLatest2<
      List<Gasto>,
      List<RecorrenciaConfiguracao>,
      List<RecorrenciaAtiva>
    >(
      ...
      (gastos, configuracoes) {
        ...
        for (final entry in grupos.entries) {
          ...
          final rec = _mapearGrupoParaRecorrencia(
            recorrenciaId: entry.key,
            grupo: entry.value,
            hoje: hoje,
            config: config,
          );
        ...
        recorrencias.sort((a, b) {
          if (a.status != b.status) {
            return a.status == RecorrenciaStatus.ativa ? -1 : 1;
          }
  ...
  Future<void> confirmarRecorrencia(RecorrenciaAtiva recorrencia) =>
      _configService.confirmarRecorrencia(recorrencia.id);
  ...
  Future<void> reativarRecorrencia(
    ...
    final datas = _gerarProximasDatas(
      aPartirDe: _inicioHoje(),
      diaDoMes: recorrencia.diaDoMes,
      quantidade: mesesFuturos,
    );

    // OTIMIZAÇÃO: Criação em lote em paralelo
    final criacaoFutures = datas.map((data) {
      final novo = recorrencia.gastoReferencia.copyWith(id: '', data: data);
      return _repository.adicionarGasto(novo);
    });

    await Future.wait(criacaoFutures);
  }

  Future<void> removerProximosLancamentos(RecorrenciaAtiva recorrencia) =>
      _removerGastos(recorrencia.ativosDesdeHoje);

  Future<void> removerRecorrenciaCompletamente(
    RecorrenciaAtiva recorrencia,
  ) async {
    await _removerGastos(recorrencia.ativosDesdeHoje);
  ...
  Future<void> atualizarNotificacao({
    required RecorrenciaAtiva recorrencia,
    required bool ativa,
    required int diasAntes,
  }) {
    return _configService.atualizarNotificacao(
  ...
  Map<String, List<Gasto>> _agruparGastos(List<Gasto> gastos) {
    ...
    for (final gasto in gastos) {
      ...
      final categoria = TextNormalizer.normalizeForSearch(
  ...
  RecorrenciaAtiva? _mapearGrupoParaRecorrencia({
    required String recorrenciaId,
    required List<Gasto> grupo,
    required DateTime hoje,
    required RecorrenciaConfiguracao? config,
  }) {
    final ordenado = List<Gasto>.from(grupo)
      ..sort((a, b) => a.data.compareTo(b.data));
    final sugestao = _recorrenciaDespesaService.detectarMensal(ordenado);
    if (sugestao == null) return null;

    final ativosDesdeHoje =
        ordenado.where((g) => !g.data.isBefore(hoje)).toList()
          ..sort((a, b) => a.data.compareTo(b.data));
    ...
    final proximoVencimento =
        proximo?.data ??
        _calcularProximoVencimento(hoje, sugestao.diaPreferencial);
    ...
    return RecorrenciaAtiva(
      ...
      categoriaLabel:
          proximo?.categoriaLabelExibicao ?? referencia.categoriaLabelExibicao,
      diaDoMes: sugestao.diaPreferencial,
      proximoVencimento: proximoVencimento,
      origem: (config?.confirmada ?? false)
          ? RecorrenciaOrigem.manual
          : RecorrenciaOrigem.detectada,
  ...
  DateTime _calcularProximoVencimento(DateTime aPartirDe, int diaDoMes) {
    final candidatoAtual = _dataComDia(
      aPartirDe.year,
      aPartirDe.month,
      diaDoMes,
    );
  ...
  List<DateTime> _gerarProximasDatas({
  ...
  // OTIMIZAÇÃO: Deleção em lote em paralelo
  Future<void> _removerGastos(List<Gasto> gastos) async {
    final futures = gastos.map((gasto) => _repository.deletarGasto(gasto.id));
